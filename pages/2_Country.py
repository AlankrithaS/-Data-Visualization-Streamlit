...
st.set_page_config(page_title="Christmas", page_icon=favicon)
...
st.markdown(f"**Population Pyramid India {year}**")
mask = df["Year"] == year
y = df[mask]["Age"]
x1 = df[mask]["M"]
x2 = df[mask]["F"] * -1
...
st.plotly_chart(fig)
st.markdown(f"*Datasource: https://www.populationpyramid.net*")
